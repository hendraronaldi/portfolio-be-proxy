...
func ResumeAgent(query string) (models_agents.ResumeAgentResponse, error) {
	var responseBody models_agents.ResumeAgentResponse
	if config.Config.ResumeAgentURL == "" {
		return responseBody, fmt.Errorf("RESUME_AGENT_URL is not set")
	}
	...
	if err != nil {
		return responseBody, fmt.Errorf("failed to call Resume Agent: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		bodyBytes, _ := ioutil.ReadAll(resp.Body)
		return responseBody, fmt.Errorf("Resume Agent returned non-200 status: %d, body: %s", resp.StatusCode, string(bodyBytes))
	}

	if err := json.NewDecoder(resp.Body).Decode(&responseBody); err != nil {
		return responseBody, fmt.Errorf("failed to decode Resume Agent response: %w", err)
	}
